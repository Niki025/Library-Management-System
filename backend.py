...
def issue():
    ...
    cur.execute(
        "CREATE TABLE if NOT exists issue(id INTEGER NOT NULL ,title text,author text,year INTEGER,isbn INTEGER)")
    conn.commit()
    conn.close()
...
def issue_delete(id):
    conn = sqlite3.connect('books.db')
    cur = conn.cuarsor()
    cur.execute("DELETE FROM issue WHERE id=?", (
    id,))  # (int(id[0]),) was giving error if only used (id,) as in front end i removed [0] from the next code line<variable_name>[0]
    conn.commit()
    conn.close()
